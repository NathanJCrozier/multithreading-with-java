...
public class Network extends Thread {
    ...
    private static Semaphore inBufferEmptySpots, outBufferEmptySpots, inBufferFullSpots, outBufferFullSpots; //Used to keep track of the number of writable and readable spots in our buffers.
    private static Semaphore processing; //Used in transferOut and transferIn to make sure we can't do another transferIn during a transferOut.
     ...
     */
     Network( )
      { 
         ...
         inBufferEmptySpots = new Semaphore(maxNbPackets);
         outBufferEmptySpots = new Semaphore(maxNbPackets);
         inBufferFullSpots = new Semaphore(0);
         outBufferFullSpots = new Semaphore(0);
         processing = new Semaphore(1);
      }     
     ...
     */
        ...
        {
        	try {
				inBufferEmptySpots.acquire(); //Ask for permission to add to the inBuffer.
     ...
     */
         public static boolean receive(Transactions outPacket)
        {
        	 ...
        	 try {
        		 //System.out.println("\n I want to receive a transaction. There are: " + outBufferFullSpots.availablePermits() + " spots to read. [from: receive()]");
				outBufferFullSpots.acquire(); //See if there's a full spot we can receive from
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
        	 	//System.out.println("\n I have acquired permission to read this spot. There are: " + outBufferFullSpots.availablePermits() + " available spots left to read.[from: receive()]");
             	...
             	//System.out.println("\n I have finished reading from the spot, and am now adding an empty spot back for someone else to use.[from: receive()]");     
        	 outBufferEmptySpots.release(); //We've taken something out of the buffer, so it has one more space.
             return true;
        }   
     ...
     */
         public static boolean transferOut(Transactions outPacket)
        {
        	 ...
        	 try {
        		 //System.out.println( "\n I want to transferOut. There are: " + outBufferEmptySpots.availablePermits() + " spots to write to and I'm trying to acquire one.[from: transferOut()]"); 
				 processing.acquire(); //Make sure we're not already trying to transferIn before we start transferOut
        		 outBufferEmptySpots.acquire(); //Ask for permission to add to the outBuffer.
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			} 
        		//System.out.println("\n I got permission and am writing to an empty spot, so there are: " + outBufferEmptySpots.availablePermits() + " spots left to write to now. [from: transferOut()]");
        	...
        	//System.out.println("\n I have finished writing to the spot, and am now adding a full spot back for someone else to use.[from: transferOut()]");     
        	processing.release(); 
        	outBufferFullSpots.release();           
             return true;
        }   
     ...
     */
       public static boolean transferIn(Transactions inPacket) {
    	   ...
    	   if (Network.getInBufferStatus().equals("empty")) { //If the inBuffer is empty, let's try sleeping for a ms to see if we're just too early.
    		   try {
				Thread.sleep(1);
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
    	   ...
    	   if (Network.getClientConnectionStatus().equals("disconnected")) { //Make sure we don't enter transferIn and then the client disconnects.
    		   //System.out.println("\n I tried to transferIn but the client was disconnected");
    		   return false;
    	   }
        }
    	  ...
    	  try {
    		 //System.out.println( "\n I want to transferIn. There are: " + inBufferFullSpots.availablePermits() + " spots to read and I'm trying to acquire one.[from: transferIn()]"); 
    		 if (Client.doneSending == true && Network.getInBufferStatus().equals("empty")) { //If the client is done sending, and there's nothing in the inBuffer, there will never be anything in the inBuffer so stop trying to acquire.
    			 //System.out.println("\n I just tried to acquire a full spot, but I now realize the client has finished sending and there will never be another full spot for me to read :(");
    			 return false;
    		 }
    		processing.acquire();	//Make sure we're not currently doing a transferOut before we transferIn.
			inBufferFullSpots.acquire(); //Grab a readable spot.
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	//System.out.println("\n I got permission and am reading a full spot, so there are: " + inBufferFullSpots.availablePermits() + " spots left to read now. [from: transferIn()]");
    		...
    		//System.out.println("\n I have finished reading the spot, and am now adding an empty spot back for someone else to use.[from: transferIn()]");     
            processing.release();
    		inBufferEmptySpots.release(); //We have successfully taken an item out of the buffer, therefore there is one more extra space.
             return true;
        }   
     ...
     */
	    public void run() {
	        ...
	        while (true) {
	            ...
	            if (getClientConnectionStatus().equals("disconnected") && getServerConnectionStatus().equals("disconnected")) { // If both server and client are
	                                                                                // disconnected, terminate. Otherwise
	                                                                                // yield.
