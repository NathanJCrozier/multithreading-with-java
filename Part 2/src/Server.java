...
public class Server extends Thread {
	...
	public static boolean threadOneDone = false, threadTwoDone = false; //Used to check whether both threads are done before disconnecting.
     ...
     */
     ...
     {   int accIndex;             	/* Index position of account to update */
        ...
        // System.out.print("\n " + this.serverThreadId + " server has started processing transactions!");
         ...
         while ((!Network.getClientConnectionStatus().equals("disconnected")))
         {
        	...
        	//	 Thread.yield(); 	/* Yield the cpu if the network input buffer is empty */
        	 ...
        	 //{ 
        		 ...
        		 if (Network.transferIn(trans) == false) {
        			 return true;
        		 }
        			 								/* Transfer a transaction from the network input buffer */
        		// System.out.println("\n The client is: " + Network.getClientConnectionStatus() + " and I'm trying to find an account!");
     ...
     */
     ...
     {  double curBalance;      /* Current account balance */
       synchronized (account[i]) { //Begin synchronized block
     ...
     */
     ...
     public void run() {
         ...
         if (this.getServerThreadId().equals("primary")) {
             Transactions trans = new Transactions();
             long serverStartTime, serverEndTime;
             System.out.println(
                     "\n DEBUG : Server.run() - starting primary server thread " + Network.getServerConnectionStatus());
             serverStartTime = System.currentTimeMillis(); // Get the start time.
             this.processTransactions(trans); // Process the transactions until there's nothing left, then proceed to
                                                 // disconnect.

             serverEndTime = System.currentTimeMillis(); // Get the end time.
             System.out.println("\n Terminating primary server thread - " + " Running time "
                     + (serverEndTime - serverStartTime) + " milliseconds");
             threadOneDone = true;
         }

         if (this.getServerThreadId().equals("secondary")) {
             Transactions trans = new Transactions();
             long serverStartTime, serverEndTime;
             System.out.println("\n DEBUG : Server.run() - starting secondary server thread "
             ...
             serverEndTime = System.currentTimeMillis(); // Get the end time.
             System.out.println("\n Terminating secondary server thread - " + " Running time "
                     + (serverEndTime - serverStartTime) + " milliseconds");
             threadTwoDone = true;
         }
